...
# - OS logo from dashboardicons.com via jsDelivr CDN
...
build_notes() {
  ...
  if [[ "$PVE_NOTES_ICONS" == "1" ]]; then
    local spec slug ext url
    spec="$(os_icon_spec "$os")" || true
    if [[ -n "${spec:-}" ]]; then
      slug="${spec% *}"
      ext="${spec#* }"
...
# Map an OS name to a dashboardicons slug + file extension.
# Echos "slug ext" (or nothing if no match).
os_icon_spec() {
  local raw="${1:-}"
  local os_lc
  ...
  case "$os_lc" in
    ...
    *alpine*)          echo "alpine-linux svg"; return;;
    *opensuse*|*sles*|"*open suse*"|*suse*) echo "opensuse svg"; return;;
  esac
